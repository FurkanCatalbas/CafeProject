...
const staffMenu = [
  { path: '/staff/order', icon: ClipboardList, label: 'Sipariş Al' },
  { path: '/orders', icon: ShoppingCart, label: 'Siparişler' },
];
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onToggle }) => {
  ...
  const menuItems = isStaff ? staffMenu : managerMenu;
      ...
      `}>
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between h-16 px-5 border-b border-blue-900/60">
            ...
            <button onClick={onToggle} className="lg:hidden text-slate-300 hover:text-white">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="mt-auto border-t border-blue-900/60 p-3.5">
            <div className="rounded-xl bg-white/10 p-2.5">
              <p className="text-sm font-medium text-white">
                {role === 'WAITER' ? 'Garson' : role === 'CASHIER' ? 'Kasiyer' : role === 'ADMIN' ? 'Yönetici' : 'Müdür'}
              </p>
